...
class SinglyLinkedListVisualizer:
    def __init__(self, root):
        ...
        insert_btn = tk.Button(control_frame, text="Insert",
    ...
    def insert_node(self):
        try:
            ...
            messagebox.showerror("Input Error",
                                 "Please enter valid integers for value and index.")
            return
        if index < 0:
    ...
    def delete_node(self):
        try:
            ...
            messagebox.showerror("Input Error",
                                 "Please enter a valid integer value to delete.")
            return
    ...
    def search_node(self):
        try:
            ...
            messagebox.showerror("Input Error",
                                 "Please enter a valid integer value to search.")
            return
            ...
            if i < len(nodes):
                ...
                if nodes[i] == val:
                    self.status_label.config(
                        text=f"Value {val} found at index {i}")
                    return
                self.root.after(500, lambda: highlight_next(i + 1))
            else:
                self.status_label.config(
                    text=f"Value {val} not found in the list")
                self.draw_list()
...
if __name__ == "__main__":
    ...
    for v in [10, 20, 30, 40]:
        app.dll.insert_at_index(v, app.dll.search(v) + 1 if app.dll.search(
            v) != -1 else 100)  # Insert at end
    app.draw_list()
